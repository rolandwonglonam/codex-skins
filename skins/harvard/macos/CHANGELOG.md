...
## 1.0.1-harvard2 — 2026-07-18

- 移除首页 Harvard 标识周围的黑色卡片和金色描边。
- 侧栏校徽改为浅色圆形底，不再使用黑色方框。
- 设置页只显示校徽，不再放置黑色 Logo 卡片。
- Mac 图标改为浅色底和单独校徽。

## 1.0.1-harvard2 — 2026-07-18

- 从 UQ `1.1.1-uq25` 视觉基线复制为独立版本。
- 使用 Harvard Crimson `#A51C30`、黑色和浅米色。
- 使用 Harvard 官方网站中的 Harvard University 签名和 Veritas 校徽。
- 首页改用 Widener Library 正面柱廊实景图。
- 保留浅色内容区、深色卡片白字、设置页配色、侧栏按钮避让和 16ms 对比同步规则。
- 使用独立安装目录和安装包名称，不覆盖 UQ 文件。
