...
if [ "$CREATE_LAUNCHERS" = "true" ]; then
  ...
  screenshot="$(shell_quote "$HOME/Desktop/Codex UQ Skin Verification.png")"
  write_launcher "$HOME/Desktop/Codex UQ Skin.command" "exec $start_script --port $PORT --prompt-restart"
  write_launcher "$HOME/Desktop/Codex UQ Skin - Customize.command" "exec $customize_script"
  write_launcher "$HOME/Desktop/Codex UQ Skin - Verify.command" "$verify_script --screenshot $screenshot && /usr/bin/open $screenshot"
  write_launcher "$HOME/Desktop/Codex UQ Skin - Restore.command" "exec $restore_script --restore-base-theme --restart-codex"
fi

printf 'Codex UQ Skin %s installed at %s for Codex %s using its signed Node.js %s.\n' \
