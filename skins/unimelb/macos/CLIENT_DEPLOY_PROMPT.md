...
使用方法：把完整的 `Codex UniMelb Skin.zip`、客户喜欢的主题图片（可选）和下面整段提示词，一起发送给客户自己的 macOS Codex。

```text
你是我这台 Mac 上的 Codex 主题部署工程师。请直接完成部署、自检和实机验收，不要只给教程，也不要在没有验证证据时声称完成。

我随消息上传了“Codex UniMelb Skin.zip”。如果还有图片附件，请把图片作为我的主题素材；如果没有图片，使用项目自带的 UniMelb 主题。
...
2. 解压后的客户目录根部应能看到：
   - `Codex.app`
   - `安装 Codex UniMelb Skin.command`
   - `使用说明.txt`
   - `给 Codex 的部署提示词.md`
   完整引擎位于 `Codex.app/Contents/Resources/engine`。不要拆开应用内容，也不要只复制其中的 CSS 或图片。

3. 将应用内的引擎记为 `<ENGINE>`，先完整阅读：
   - `<ENGINE>/README.md`
   - `<ENGINE>/SKILL.md`
...
4. 确认官方 Codex 至少运行过一次，且 `~/.codex/config.toml` 已存在。运行：
   ...
   完整项目应被安装到 `~/.codex/codex-unimelb-skin-studio`，并生成桌面启动、定制、验证和恢复入口。

5. 如果我上传了主题图片，使用安装后的脚本处理素材：
   `~/.codex/codex-unimelb-skin-studio/scripts/customize-theme-macos.sh --image "<图片绝对路径>" --name "我的 Codex Dream Skin" --no-apply`
...
7. 启动后必须运行：
   - `~/.codex/codex-unimelb-skin-studio/scripts/doctor-macos.sh --require-live`
   - `~/.codex/codex-unimelb-skin-studio/scripts/verify-dream-skin-macos.sh --reload --screenshot "<首页验收截图绝对路径>"`
   验证器必须真实返回 `pass: true`。随后还要检查一个正常任务页面，确认背景存在且正文、菜单、侧栏和输入框仍清晰可用，并保存任务页截图。

8. 检查桌面已存在以下四个入口：
   - `Codex UniMelb Skin.command`
   - `Codex UniMelb Skin - Customize.command`
   - `Codex UniMelb Skin - Verify.command`
   - `Codex UniMelb Skin - Restore.command`
