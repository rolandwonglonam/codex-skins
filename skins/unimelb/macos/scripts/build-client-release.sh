...
OUTPUT="${1:-$HOME/Desktop/Codex UniMelb Skin.zip}"
TMP="$(/usr/bin/mktemp -d /tmp/codex-unimelb-client.XXXXXX)"
CLIENT_ROOT="$TMP/Codex UniMelb Skin"
...
/usr/bin/printf '%s\n' \
  ...
  > "$CLIENT_ROOT/安装 Codex UniMelb Skin.command"

/usr/bin/printf '%s\n' \
  'Codex UniMelb Skin 1.0.0-unimelb1' \
  '' \
  '双击“Codex.app”即可安装并加载皮肤。以后再点它，会直接启动 UniMelb 皮肤。' \
  '' \
  '如果 macOS 第一次拦截，请右键“Codex.app”，选择“打开”。' \
  '' \
  '备用方式：双击“安装 Codex UniMelb Skin.command”。' \
  '' \
  'Codex.app 内含完整运行引擎。不要拆开应用内容。' \
  > "$CLIENT_ROOT/使用说明.txt"

/bin/cp "$ROOT/CLIENT_DEPLOY_PROMPT.md" "$CLIENT_ROOT/给 Codex 的部署提示词.md"
/bin/chmod 755 "$CLIENT_ROOT/安装 Codex UniMelb Skin.command"
