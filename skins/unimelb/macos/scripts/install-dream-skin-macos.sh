...
"$SCRIPT_DIR/install-auto-inject-macos.sh" --port "$PORT"
...
if [ "$CREATE_LAUNCHERS" = "true" ]; then
  ...
  screenshot="$(shell_quote "$HOME/Desktop/Codex UniMelb Skin Verification.png")"
  write_launcher "$HOME/Desktop/Codex UniMelb Skin.command" "exec $start_script --port $PORT --prompt-restart"
  write_launcher "$HOME/Desktop/Codex UniMelb Skin - Customize.command" "exec $customize_script"
  write_launcher "$HOME/Desktop/Codex UniMelb Skin - Verify.command" "$verify_script --screenshot $screenshot && /usr/bin/open $screenshot"
  write_launcher "$HOME/Desktop/Codex UniMelb Skin - Restore.command" "exec $restore_script --restore-base-theme --restart-codex"
fi

printf 'Codex UniMelb Skin %s installed at %s for Codex %s using its signed Node.js %s.\n' \
...
printf 'Automatic theme loading is enabled when Codex is opened normally.\n'
